...
const _primary = Color(0xFF4C63E7);
          ...
          children: [
            Row(
              children: [
                ...
                if (MediaQuery.sizeOf(context).width > 650) const _StoreChip(),
              ],
            ),
            const SizedBox(height: 18),
            SearchBox(
              hint: 'Search products, brands, models, SKU or barcode',
              onScan: () => _barcodeDialog(context),
            ),
            const SizedBox(height: 16),
            _HeroBanner(products: products.length, onAdd: widget.onInventory),
            const SizedBox(height: 24),
...
class _HeroBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [Color(0xFF202A63), _primary]),
...
class _Metric extends StatelessWidget {
  ...
  final Color accent;
  final String? note;
  const _Metric(this.title, this.value, this.icon, this.accent, {this.note});
...
class _ProductImageState extends State<ProductImage> {
  ...
  void initState() {
    super.initState();
    final saved = widget.product.imageUrl.trim();
    url = saved.isEmpty ? null : saved;
    if (url == null) _lookup();
  }
...
class InventoryPage extends StatefulWidget {
  const InventoryPage({super.key, this.category});
...
class _InventoryPageState extends State<InventoryPage> {
  ...
  Future<void> _showBarcodeSearch(BuildContext context) async {
